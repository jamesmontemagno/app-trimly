			...
			Form {
				...
				Section("Goal") {
					if let goal = dataManager.fetchActiveGoal() {
						...
						Button("Change Goal") { showingGoalSheet = true }
						Button("Goal History") { showingGoalHistory = true }
					} else {
						Button("Set Goal") { showingGoalSheet = true }
					}
				}
				...
				Section("Data") {
					Button("Export Data (CSV)") { exportData() }
